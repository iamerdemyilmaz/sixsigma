...
| m14-smed-changeover | 14 | two-sample t (existing kind `ttest2`) | furnace-fixture changeover time (min), 16 before/16 after SMED: mean 68.54→26.77 min, Levene p=0.0084 (unequal variances), Welch t=13.31 df=18.9 p<0.0001, Cohen's d=4.70 | `m14-smed-changeover.csv` | pass | pass | pass | pass (`Stats.ttest2`, "tests" calc) | 14 |
| m14-pugh-fixture | 14 | Pugh matrix (new kind `pugh_matrix`) | 5 furnace-fixture concepts scored on 5 criteria vs. baseline: Modular quick-change wins (net +3, not the cheapest); Ceramic insert nets −2 despite improving the one criterion the team cared most about | `m14-pugh-fixture.csv` | pass | pass | pass | n/a (no calculator) | 14 |
| m14-pilot-before, m14-pilot-after | 14 | p charts (existing kind `p`) | leak-test pilot, 15 lots before (p̄=4.41%) / 14 lots after (p̄=2.01%) a fixture change; both in control on their own, LCL floors at 0.000 in both — cannot itself signal the improvement | `m14-pilot-before.csv`, `m14-pilot-after.csv` | pass | pass | pass | pass (`Stats.pChart`, "control" calc) | 14 |
| m14-pilot-chisq | 14 | 2×2 chi-square (existing kind `chisq`; new fields `relative_risk`/`risk_difference_pct` added to the shared kind) | totals of the pilot before/after: χ²=10.524, p=0.0012, relative risk 0.455, risk difference −2.41 pp, Cramér's V 0.064 (small, despite the practically large relative-risk drop — a rare-event-table caution) | none | pass | pass | pass | n/a (`chisq` has no calculator) | 14 |
| m14-ex1-changeover | 14 | two-sample t | pallet-changeover time, 10 before/10 after a locating poka-yoke: mean 16.00→9.82 min, t=3.59 df=18 p=0.0021, d=1.60 (a smaller, more marginal effect than the main example, needing the t-table rather than being obvious); exercise 1 | `m14-ex1-changeover.csv` | pass | pass | pass | pass (`Stats.ttest2`) | 14 |
| m14-ex2-pugh | 14 | Pugh matrix | connector poka-yoke, 3 concepts vs. baseline on 4 criteria: Mechanical stop pin wins (net +3); Optical sensor interlock nets −1 despite matching the winners on error prevention alone; exercise 2 | `m14-ex2-pugh.csv` | pass | pass | pass | n/a (no calculator) | 14 |

The `relative_risk`/`risk_difference_pct` fields added to the shared `chisq` kind for Module 14 (a 2×2-only companion effect size, guarded by `if r == 2 and c == 2`) also apply retroactively to the pre-existing `m11-chisq-shift`, `m11-ex2-chisq-leak`, and `m19-before-after` examples, all already 2×2 tables; re-running the full suite confirmed all three still pass with the new fields present.

Totals on 2026-09-10 (after Module 14, both sessions combined): 153 examples; 4,159 quantities agree between Check 1 and Check 2; 8,488 values agree between Check 1 and the JavaScript engines (118 examples with a calculator engine); 1,415 page checks.

| m15-stack-three-parts | 15 | tolerance stack-up (new kind `tolerance_stack`) | 3-part gap stack, spec 2.000±0.100mm: worst case 0.115mm (exceeds spec), RSS 0.0687mm (within spec); predicted Cpk 1.455 (centred, Cpk=1.0/part) vs simulated Cpk 0.901 (200,000-run Monte Carlo, 1σ per-part centring bias, ~3,530 ppm out of spec) — RSS's spread prediction held almost exactly, the centring assumption did not | none (parameter-only) | pass | pass | pass | n/a (no calculator) | 15 |
| m15-alloc-three-parts | 15 | tolerance stack-up | same stack, tolerance reallocated (A loosened 0.050→0.060, B tightened 0.025→0.015): worst case ~unchanged (0.115), RSS rises only to 0.0737mm, still within spec; predicted Cpk 1.806, simulated Cpk 1.544 (0.5σ bias, Cpk=1.33/part) | none | pass | pass | pass | n/a | 15 |
| m15-loss-bore | 15 | Taguchi loss (new kind `taguchi_loss`) | reuses m07-bore's 125 values, target 12.000mm, A₀=40 (illustrative cost units), δ₀=0.025mm: k=64000, avg loss 3.879/part (25.15% from off-centring), total 484.86; worst single reading's loss = exactly A₀ (that bore sat exactly at the spec limit) | none (reuses `m07-bore.csv`) | pass | pass | pass | n/a | 15 |
| m15-ex1-stack | 15 | tolerance stack-up | 2-part clearance stack, spec 0.050±0.045mm: worst case 0.050 (exceeds), RSS 0.0361 (within); predicted Cpk 1.660 vs simulated Cpk 1.428 (0.5σ bias, Cpk=1.33/part, ~20 ppm); exercise 1 | none | pass | pass | pass | n/a | 15 |
| m15-ex2-loss | 15 | Taguchi loss | fill weight (g), n=25, target 250.0g, A₀=20, δ₀=3.0g: mean 249.205g, k=2.222, avg loss 4.152 (33.81% from off-centring, a materially larger share than the bore example); exercise 2 | `m15-ex2-loss.csv` | pass | pass | pass | n/a | 15 |

One real bug caught by `sanity.py`, not by review: Exercise 2's 25-value fill-weight table was first drafted with plausible-looking but entirely hand-fabricated numbers (never actually extracted from the generated CSV), and `sanity.py`'s free-number scan correctly flagged 15 of them as untraceable. Fixed by reading `m15-ex2-loss.csv` directly and replacing the table with the exact values. `tolerance_stack`'s Monte Carlo fields use a genuinely independent route in `recompute.py`: `compute.py` draws with `numpy`, `recompute.py` draws with the `_XorShift` generator already used for Module 16's `arl` kind (its own seed, unrelated to `compute.py`'s), and only `simulated.mean`/`simulated.cpk` are cross-route-compared (via a new `simulated.` case in `close()`, tolerance 3%, about 15× the empirical run-to-run sd observed across 10 independent 200,000-draw replications); `simulated.sd`/`simulated.ppm` are range/reasonableness-checked by `sanity.py` against `compute.py`'s own output instead, since their Monte Carlo standard error is too large a fraction of their own scale for a single shared tolerance to be honest for both them and `mean`/`cpk` at once.

Totals on 2026-09-10 (after Module 15, both sessions combined): 158 examples; 4,196 quantities agree between Check 1 and Check 2; 8,488 values agree between Check 1 and the JavaScript engines (118 examples with a calculator engine); 1,451 page checks.

| m18-month1..4 | 18 | ongoing capability (existing kind `capability`, no new kind) | same bore feature as m07-bore, 6 subgroups of 5 each, 4 separate "months": Ppk 1.029, 1.037, 1.077, 1.237 — point estimates wander but all four 95% CIs overlap heavily (widest: month 4's 0.897–1.577 alone covers every other month's point estimate) | none (parameter-generated) | pass | pass | pass | n/a | 18 |
| m18-month5 | 18 | ongoing capability | same feature, 5th month, a genuine shift (mean 12.0124mm vs ~12.003mm, s 0.00813 vs ~0.007mm): Ppk 0.517, CI (0.338, 0.695) — does not overlap any of months 1–4's lower bound (lowest is 0.739) | none | pass | pass | pass | n/a | 18 |
| m18-ex2-montha/b/c | 18 | ongoing capability | injection-moulded wall thickness, 3 months: A (Ppk 1.266) and B (Ppk 1.034) overlap; C (Ppk 0.450, CI 0.268–0.633) does not overlap B's lower bound (0.714) — a real shift; exercise 2 | none | pass | pass | pass | n/a | 18 |
