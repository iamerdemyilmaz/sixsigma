# Claude Code prompt: Six Sigma, process capability and SPC course

How to use this file:

1. Create an empty folder (for example `six-sigma-course/`) and save this file inside it as `CLAUDE.md`. Claude Code reads it at the start of every session, so the rules survive context resets.  
2. Make sure Python 3 with `numpy`, `scipy`, `pandas`, and `statsmodels` is installed in that environment. The verification protocol below depends on it. If Claude Code cannot install them, stop and tell the author before writing any content.  
3. Open Claude Code in the folder and paste the kickoff prompt at the bottom of this file.  
4. Expect several sessions. `PROGRESS.md`, `SOURCES.md`, and `EXAMPLES.md` are the handoff between sessions.
...
# Kickoff prompt (paste into Claude Code after saving the above as CLAUDE.md)

Read CLAUDE.md fully. Print your model recommendation line for Phase A. First confirm that Python 3 with numpy, scipy, pandas, and statsmodels, and Node, are available in this environment; install them if not, and stop if you cannot. Then start Phase A: for each of the 20 modules, search for and read sources, and produce SOURCES.md with at least 5 verified sources per module and a one-paragraph outline per module, listing for each module the real-world examples you plan to use with their sources and the invented examples you plan to construct with their engineering setting and the point each makes. Do not write any HTML or verification code yet. When done, summarize the outline, list any modules where you could not find a verifiable real-world example, list the controversies you intend to cover and the sources on each side, then stop and wait for my approval.

# Standing resume prompt (paste at the start of every later session)

Read CLAUDE.md and the top of PROGRESS.md, then continue from the Resume here line. Print your model recommendation line first.  
